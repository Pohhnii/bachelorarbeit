...
class DenseLayer(private val inputs: Int, private val nodes: Int) : Layer() {
    ...
    override fun backpropagation(
        ...
        val dws = mutableListOf<D2Array<Double>>()
        val dxs = mutableListOf<D2Array<Double>>()

        for (i in inputs.indices) {
            dws.add(inputs[i].transpose().dot(errors[i]))
            dxs.add(errors[i].dot(weights.transpose()))
        }

        this.data = listOf(
            weights + (dws.reduce { acc, dx -> acc + dx } * learnRate), // weights
            bias + (errors.reduce { acc, err -> acc + err } * learnRate) // bias
        )
